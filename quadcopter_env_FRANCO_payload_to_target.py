...
# Pre-defined configs
##
from .drone_FRANCO import DRONE_PAYLOAD_CFG  # isort: skip
...
class QuadcopterEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        #payload_pos_w = self._robot.data.body_state_w[:, self._payload_idx, :3]
        #payload_vel_w = self._robot.data.body_state_w[:, self._payload_idx, 7:10]
        #payload_speed = torch.sum(torch.square(payload_vel_w), dim=1)
   ...
   # def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        final_distance_to_goal = torch.linalg.norm(
            self._desired_pos_w[env_ids] - self._robot.data.body_state_w[env_ids, self._payload_sphere_id, :3], dim=1
        ).mean()
        extras = dict()
